...
@section('content')
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden max-w-2xl mx-auto">
        ...
        <div class="px-8 py-6 border-b border-gray-100 bg-gray-50/50">
            <h3 class="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Layout Hook Configuration</h3>
        </div>
        ...
        <form action="{{htcms_get_save_path(request()->module_info->controller_name)}}" method="post" id="addEditForm">
            <div class="p-8 lg:p-10 space-y-6">
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        {!! FormHelper::label('name', 'Hook Name', array('class' => 'text-sm font-medium text-slate-700 block')) !!}
                    ...
                    <div class="space-y-2">
                        {!! FormHelper::label('alias', 'Hook Alias (Unique)', array('class' => 'text-sm font-medium text-slate-700 block')) !!}
                    ...
                    <div class="space-y-2">
                        {!! FormHelper::label('description', 'Contextual Description', array('class' => 'text-sm font-medium text-slate-700 block')) !!}
                        {!! FormHelper::textarea('description', $description , array('class'=>'form-control w-full bg-white border transition-all duration-300 outline-none font-bold text-xs tracking-tight py-3.5 rounded-xl px-4 pl-3 hover:border-gray-300 border-gray-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-gray-900', 'required'=>'required', 'rows' => 3)) !!}
                    </div>

                    <div class="space-y-2">
                        {!! FormHelper::label('direction', 'Layout Direction', array('class' => 'text-sm font-medium text-slate-700 block')) !!}
                        {!! FormHelper::select('direction', $directions, array("class"=>"form-select w-full bg-white border transition-all duration-300 outline-none font-bold text-xs tracking-tight py-3.5 rounded-xl px-4 pl-3 hover:border-gray-300 border-gray-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-gray-900"), $direction, "plain_array","") !!}
                    </div>
                </div>
            </div>
