...
@section('content')
    ...
    <div
        ...
        <div class="px-8 py-6 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
            ...
            <span class="text-[10px] font-black uppercase text-blue-600 bg-blue-50 px-3 py-1 rounded-full">Global
                Identity</span>
        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="text-[10px] text-slate-400 font-medium">
                                The primary domain for this site. A single site can be served on <span
                                    class="font-black text-slate-500">multiple domains</span> — configure all of them via
                                <code class="bg-slate-100 px-1 py-0.5 rounded text-slate-600">config/hashtagcms.php</code> →
                                <code class="bg-slate-100 px-1 py-0.5 rounded text-slate-600">domains</code>.
                            </p>
                        </div>
                            ...
                            <div class="mt-2 rounded-xl border border-blue-100 bg-blue-50/50 overflow-hidden">
                                <div class="px-4 py-2.5 flex items-start gap-2.5">
                                    ...
                                    <div class="space-y-1.5">
                                        <p class="text-[10px] font-black uppercase tracking-widest text-blue-600">How
                                            Multi-Domain Works</p>
                                        <p class="text-[10px] text-blue-700/80 leading-relaxed">
                                            A single site can run on multiple domains. Each domain must map to this site's
                                            <strong>Context</strong> value inside
                                            <code class="bg-blue-100 px-1 py-0.5 rounded">config/hashtagcms.php</code>:
                                        </p>
                                        <pre
                                            class="mt-1.5 bg-blue-900/10 text-blue-800 text-[10px] font-mono rounded-lg px-3 py-2 leading-relaxed overflow-x-auto">'domains' => [
                    'dev.hashtagcms.com'     => env('CONTEXT', '{{ $context ?? 'htcms' }}'),
                    'www.myothersite.com'    => '{{ $context ?? 'htcms' }}',
                ]</pre>
                                        <p class="text-[10px] text-blue-600/70">The value on the right of each domain entry
                                            must match the Context you enter above.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center gap-4 bg-red-50/50 p-5 rounded-xl border border-red-100">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[10px] text-red-600/70 font-medium tracking-tight">Public access will
                                        be restricted to a maintenance message</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                @if($id > 0)
                    ...
                    <section class="pt-10 border-t border-slate-50 space-y-8">
                        <div class="flex items-center gap-3">
                            ...
                            <h4 class="text-[10px] font-black uppercase tracking-widest text-slate-800">Localization & Defaults
                            </h4>
                        </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-2">
                                {!! FormHelper::label('currency_id', 'Currency', array('class' => 'text-sm font-medium text-slate-700 block')) !!}
                                {!! FormHelper::select('currency_id', $currencies, array('class' => 'form-select w-full bg-white border transition-all duration-300 outline-none font-bold text-xs tracking-tight py-3.5 rounded-xl px-4 pl-3 hover:border-gray-300 border-gray-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-gray-900'), $currency_id, array("label" => "name", "value" => "id")) !!}
                            </div>
                        </div>
                    </section>
                @endif
