...
class BlogController extends FrontendBaseController
{
    ...
    private function fetchBlogsFromExternalApi($category, $limit)
    {
        ...
        $cacheKey = "hashtagcms_external_blog_{$context}_{$catKey}_{$limit}";
        $cacheTTL = config('hashtagcms.external_data_cache_ttl', 30);

        return Cache::remember($cacheKey, now()->addMinutes($cacheTTL), function () use ($apiUrl, $apiSecret, $context, $category, $limit) {
